...
import CreateToken from "./createNFT";
import LoadingAnimation from "./animation/loadingAnimation";
import Animation from "./animation/emptyAnimation";
import NavbarComponent from "./navbar";
var obj
const NFTDashboardEntries=(props) =>{
...
const Dashboard = (props) => {
  ...
  function renderCoinDetails() {
    if (props?.data?.NFT?.length === 0 || props.data === null)
      return (
        <div className="text-center py-3">
          <Animation path="./noData.json" />
          <br />
          <h4 style={{ color: "white" }}>No NFT available</h4>
        </div>
      );
    else {
      return props.data?.NFT?.map((item) => {
        return (
          <Row
            style={{
      ...
      {!show && (
        ...
        >
          <div className="p-3" style={{ textAlign: "center" }}>
            <h4 style={{ color: "white" }}>NFT Management</h4>
            <hr style={{ color: "white" }} />
          </div>
          <Row className="m-3">
            <h5 style={{ color: "white" }}>NFT in your wallet: </h5>
            <p style={{ color: "white" }}>
              Any NFT's minted using the address connected to Solminter will
...
export default NFTDashboardEntries;
